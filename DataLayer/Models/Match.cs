...
namespace DataLayer.Models
{
    public class Match : IComparable<Match>
    {
        ...
        public static Match[] FromJson(string json)
            => JsonConvert.DeserializeObject<Match[]>(json, MatchConverter.Settings);

        public override string ToString()
            => $"{Venue}, {LastEventUpdateAt.Date:dd/MM/yyyy}";

        public string GetPrintString()
            => $"Venue: {Venue}" + Environment.NewLine +
                $"Home team: {HomeTeamCountry}" + Environment.NewLine +
                $"Away team: {AwayTeamCountry}" + Environment.NewLine +
                $"Attendance: {Attendance}";

        public int CompareTo(Match other)
            => -Attendance.CompareTo(other.Attendance);
    }
